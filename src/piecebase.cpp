...
const QList<PieceColor> PieceBase::ALLCOLORS { PieceColor::RED, PieceColor::BLACK };
...
const QString PieceBase::FENSTR { "rnbakabnr/9/1c5c1/p1p1p1p1p/9/9/P1P1P1P1P/1C5C1/9/RNBAKABNR" };
...
QString PieceBase::getIccsChars()
{
    return ICCSROWCHARS + ICCSCOLCHARS;
}
...
bool PieceBase::isPiece(QChar name)
{
    return isKind(name, ALLKINDS);
}
...
int PieceBase::getRowFrom(QChar ch)
{
    return ICCSROWCHARS.indexOf(ch);
}

int PieceBase::getColFrom(QChar ch)
{
...
{
    return color == PieceColor::RED ? L'红' : L'黑';
}
...
{
    return CHARS.at(int(PieceColor::RED)).contains(ch) ? PieceColor::RED : PieceColor::BLACK;
}
...
int PieceBase::getIndex(const int seatsLen, const bool isBottom, QChar preChar)
{
...
QChar PieceBase::getIccsChar(int col)
{
