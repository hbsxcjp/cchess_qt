...
public:
    ...
    // 设置棋谱对象的开局名称
    bool setECCO(ChessManual* ins);
    bool setECCO(QList<ChessManual*> insList);
...
private:
    // 初始化开局库的辅助函数
    static QString getRowcols_(const QString& zhStr, ChessManual& ins, bool isGo);
    static QStringList getRowcolsList_(const QString& mvstr, bool order, ChessManual& ins);
    static QString getColorRowcols_(const QString& mvstrs, const QString& anyMoveRegStr, ChessManual& ins);
    static QString getRegStr_(const BoutStrs& boutStrs, const QString& sn, ChessManual& ins);
    ...
    QStringList getECCO(ChessManual* ins);

    // 获取棋谱对象链表
    static QList<ChessManual*> getInsList_dir__(const QString& dirName);
    static QList<ChessManual*> getInsList_webfile__(const QString& insFileName);
    static QList<ChessManual*> getInsList_db__(const QString& dbName, const QString& man_tblName);

    // 存储对象的info数据至数据库（返回对象个数）
    static int storeToDB__(QList<ChessManual*> insList, const QString& dbName, const QString& tblName);
