...
ManualSubWindow::ManualSubWindow(QWidget *parent)
    : QWidget(parent), isUntitled(true), isModified(false),
      titleName_(QString()), state_(SubWinState::NOTSTATE), manual_(new Manual),
      commandContainer_(new CommandContainer), ui(new Ui::ManualSubWindow) {
  ui->setupUi(this);
...
bool ManualSubWindow::saveAs() {
  QString fileName =
      QFileDialog::getSaveFileName(this, "另存为", "./", getFilter(true));
  if (fileName.isEmpty())
    return false;
...
void ManualSubWindow::setNavButtonMenu(QToolButton *btn,
  ...
  for (auto &text : commandStrings) {
    ...
    action->setData(++num);
    auto func =
        isRevoke ? &ManualSubWindow::revokeNum : &ManualSubWindow::recoverNum;
...
void ManualSubWindow::readSettings() {
  ...
  int moveInfoIndex =
          settings.value(stringLiterals[StringIndex::MOVEINFOTABINDEX], 0)
              .toInt(),
      moveIndex =
          settings.value(stringLiterals[StringIndex::MOVETABINDEX], 0).toInt(),
      pgnTextIndex =
          settings.value(stringLiterals[StringIndex::MOVEPGNTEXTINDEX], 0)
              .toInt();
  ...
  QVariant winGeometry =
      settings.value(stringLiterals[StringIndex::WINGEOMETRY]);
  if (!winGeometry.isNull())
    getSubWindow()->restoreGeometry(winGeometry.toByteArray());

  moveSound =
      settings.value(stringLiterals[StringIndex::MOVESOUND], true).toBool();
  soundDir =
      settings
